...
namespace Jun.Data.SQL
{
    public class MySQLBuilder : ISQLBuilder, ISingletonDependency
    {
        ...
        {
            PropertyInfo idPropertyInfo = ReflectionUtil.GetPropertyInfo(entity, EntityConst.EntityIdName);
        ...
        {
            StringBuilder sql = new StringBuilder();
            PropertyInfo idPropertyInfo = ReflectionUtil.GetPropertyInfo(entitys.FirstOrDefault(), EntityConst.EntityIdName);
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("update {0} set status=1 where "+ EntityConst.EntityIdName + "={1};", tableName, id);
            return sql;
        }
        #endregion
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("update {0} set status=1 where "+ EntityConst.EntityIdName + " in({1});", tableName, string.Join(",", ids));
            return sql;
        }
        #endregion
        ...
        public StringBuilder BuildUpdateSql<T>(string tableName, T entity, params string[] columns) where T : class
        {
            ...
            sql.AppendFormat(" where "+ EntityConst.EntityIdName + "={0} ;", id);
        ...
        PropertyInfo[] GetNeedHandleProperty(object entity)
        {
            ...
            {
                //过滤ID
                if (propertyInfo.Name.ToLower() == EntityConst.EntityIdName) continue;
                //过滤 notmapped
                ...
                //过滤掉 实体
                if (propertyInfo.PropertyType.FullName.Contains(EntityConst.EntityNameSpace)&&!propertyInfo.PropertyType.IsEnum) continue;
